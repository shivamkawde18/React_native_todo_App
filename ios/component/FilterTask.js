...
const FilterTask = props => {
  return (
    <>
      {props.flag === 'completeTask' ? (
        <Text marginTop="30%" fontSize="2xl" marginLeft="20%">
          {' '}
          Completed Todos
        </Text>
      ) : (
      ...
      <View flex={0} flexDirection="row" justifyContent="space-evenly">
        {/* {props.flag==="uncompleteTask"?<Text bold color="green.700" onPress={()=>props.setCompleteTasksFlag(true)}>Complete Task</Text>:   <Text bold color="green.700" onPress={()=>{
            props.setUncompleteTasksFlag(true)
          }}>Uncomplete Task</Text>} */}
        <Text
          onPress={() => {
      ...
      {props.data.length == 0 ? (
        ...
        <ScrollView
          ...
          {props.data.map((e, index) => {
            if (
              e.taskCheck == (props.flag === 'uncompleteTask' ? false : true)
            ) {
              return (
                <>
                  <View
                    ...
                    <HStack space={6} marginRight="92%">
                      <Checkbox
                        ...
                        onChange={async check => {
                          ...
                          if (props.flag == 'completeTask') {
                            props.setCompleteTasksFlag(true);
                            props.setUncompleteTasksFlag(false);
                          } else {
                            props.setUncompleteTasksFlag(true);
                            props.setCompleteTasksFlag(false);
                          }
